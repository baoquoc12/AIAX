...
function formatUserPrompt(cfg, key, ...args) {
  ...
  const templates = {
    en: {
      ...
      prop_constraint: '**Important** — props field rules:\n1. Only use prop IDs (numbers) from the above prop list. Do not invent IDs.\n2. If a listed prop name, synonym, or clear object reference appears in action/dialogue/result/narration and is visible or handled in this shot, you MUST include that prop ID.\n3. Include props that are visually present, actively used, discovered, held, opened, shown, exchanged, hidden, stolen, or prominently featured. Example: if the shot says the character finds/holds/opens a USB and the prop list has USB Drive, props MUST contain that USB Drive ID.\n4. If no props from the list appear in the shot, use an empty array [].',
